...
class bsn_set_pktin_suppression(Message):
    ...
    def __init__(self, xid=None, enabled=None, pad=None, idle_timeout=None, hard_timeout=None, priority=None, cookie=None):
        self.xid = xid
        if enabled != None:
    ...
    def unpack(buf):
        ...
        _length = struct.unpack_from('!H', buf, 2)[0]
        assert(_length == len(buf))
        if _length != 32: raise loxi.ProtocolError("bsn_set_pktin_suppression length is %d, should be 32" % _length)
        obj.xid = struct.unpack_from('!L', buf, 4)[0]
        experimenter = struct.unpack_from('!L', buf, 8)[0]
        assert(experimenter == 0x5c16c7)
        subtype = struct.unpack_from('!L', buf, 12)[0]
        assert(subtype == 11)
        obj.enabled = struct.unpack_from('!B', buf, 16)[0]
        obj.pad = struct.unpack_from('!B', buf, 17)[0]
        obj.idle_timeout = struct.unpack_from('!H', buf, 18)[0]
        obj.hard_timeout = struct.unpack_from('!H', buf, 20)[0]
        obj.priority = struct.unpack_from('!H', buf, 22)[0]
        obj.cookie = struct.unpack_from('!Q', buf, 24)[0]
        return obj
